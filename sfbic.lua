...
script_author('Ðàçðàáîò÷èê: William Raines. Îòäåëüíàÿ áëàãîäàðíîñòü: Donny Provenzano, Gabriel Marques è Kevin Norberg')
...
function chat.sendWarning(text)
  sampAddChatMessage(string.format('[SWAT FBI {a11212}Ïðåäóïðåæäåíèå{363636}] {ffffff}%s', text), 0x363636);
end
...
function chat.sendEmployee(id, name, rank, duty, position)
  ...
  if duty == 'Íà ðàáîòå' then
    onDuty = true
    return sampAddChatMessage(string.format('# {9C3848}[%s] {ffffff}%s {9C3848}: {ffffff}%s {9C3848}|{ffffff} %s', id, name, rank, position), -1);
  end
  return sampAddChatMessage(string.format('# {9C3848}[%s] {ffffff}%s {9C3848}: {ffffff}%s {9C3848}|{ffffff} %s {ed0713}[!] Ïðîãóë', id, name, rank, position), 0x1e90ff);
end
...
function chat.showIRCMessage(user, channel, message, squad_name, ooc)
  ...
  sampAddChatMessage(string.format('{363636}%s {a11212}[Ðàöèÿ] {ffffff}| %s: %s', squad_name, user, message), -1)
end
          ...
          if f then
            ...
            if updateversion ~= thisScript().version then
              lua_thread.create(function(prefix)
                local dlstatus = require('moonloader').download_status
                local color = -1
                chat.sendMessage((prefix..'Îáíàðóæåíî îáíîâëåíèå. Ïûòàþñü îáíîâèòüñÿ c '..thisScript().version..' íà '..updateversion), color)
                wait(250)
                downloadUrlToFile(updatelink, thisScript().path,
                  function(id3, status1, p13, p23)
                    ...
                    elseif status1 == dlstatus.STATUS_ENDDOWNLOADDATA then
                      ...
                      chat.sendMessage((prefix..'Îáíîâëåíèå çàâåðøåíî!'), color)
                      goupdatestatus = true
                      lua_thread.create(function() wait(500) thisScript():reload() end)
                    end
                    if status1 == dlstatus.STATUSEX_ENDDOWNLOAD then
                      if goupdatestatus == nil then
                        chat.sendMessage((prefix..'Îáíîâëåíèå ïðîøëî íåóäà÷íî. Çàïóñêàþ óñòàðåâøóþ âåðñèþ..'), color)
                        update = false
                      end
                    end
                  end
  ...
  for key, employee in pairs(filteredMembers) do
    for key, squadmate in pairs(squadmates) do
      if squadmate:find(employee.nickname) then
        _callsign = string.match(squadmate, '| %[[À-ßà-ÿ]+%]')
        callsign = string.match(_callsign, '[À-ßà-ÿ]+')
        _organization = string.match(squadmate, '| %a%a%a?%a')
        organization = string.match(_organization, '%a%a%a?%a')
        _position = string.match(squadmate, '%([À-ßà-ÿ]+%.?[À-ßà-ÿ]+%)')
        position = string.match(_position, '[À-ßà-ÿ]+%.?[À-ßà-ÿ]+')
        _rank = string.match(squadmate, '| [À-ßà-ÿ]+%.?[À-ßà-ÿ]+')
        rank = string.match(_rank, '[À-ßà-ÿ]+%.?[À-ßà-ÿ]+')
...
function network.get(link)
  ...
  print("Äàííûå óñïåøíî çàãðóæåíû | ", code)
  return resp
end
...
function imgui.OnDrawFrame()
  ...
  if members_window_state.v then
    ...
    imgui.Begin(u8'[S-FBI] Ñïèñîê ñîòðóäíèêîâ | Âñåãî â ñåòè: ' .. #staff, members_window_state, imgui.WindowFlags.NoResize)
    ...
    imgui.NextColumn()
    imgui.TextColoredRGB('{B81E56}Íèêíåéì')
    for key, squadmate in pairs(staff) do
    ...
    imgui.NextColumn()
    imgui.TextColoredRGB('{de0012}Ïîçûâíîé')
    for key, squadmate in pairs(staff) do 
      imgui.Text(u8(squadmate.callsign))
    end
    imgui.NextColumn()
    imgui.TextColoredRGB('{e4f500}Îðãàíèçàöèÿ')
    for key, squadmate in pairs(staff) do 
    ...
    imgui.NextColumn()
    imgui.TextColoredRGB('{e4f500}Çâàíèå')
    for key, squadmate in pairs(staff) do 
      imgui.Text(u8(squadmate.rank))
    end
    imgui.NextColumn()
    imgui.TextColoredRGB('{ff4b19}Äîëæíîñòü')
    imgui.Separator()
    for key, squadmate in pairs(staff) do 
  ...
  if main_window_state.v then
    ...
    imgui.Begin(u8'SWAT FBI | Ãëàâíîå ìåíþ | Âåðñèÿ: 0.0.2', main_window_state, imgui.WindowFlags.NoResize)
    if imgui.ButtonHex(u8'Êîìàíäû ñêðèïòà', 0x6468e8, imgui.ImVec2(285, 0)) then
      commandHandlers.renderS_C()
    end
    if imgui.ButtonHex(u8'Èíôîðìàöèÿ äëÿ ñòàæ¸ðîâ', 0x6468e8, imgui.ImVec2(285, 0)) then
      commandHandlers.renderIFT()
    end
    if imgui.ButtonHex(u8'Òåí-êîäû', 0x6468e8, imgui.ImVec2(285, 0)) then
      commandHandlers.renderCodes()
    end
    if imgui.ButtonHex(u8'Îòûãðîâêè', 0x6468e8, imgui.ImVec2(285, 0)) then
      commandHandlers.renderActings()
    end
    if imgui.ButtonHex(u8'Ïðîòîêîë', 0x6468e8, imgui.ImVec2(285, 0)) then
      commandHandlers.renderProtocol()
    end
    imgui.CenterTextColoredRGB('{ff0000}[!]{ffffff} Òåñòîâûé ðåæèì')
    imgui.CenterTextColoredRGB('{ff0000}[!]{ffffff} Åñëè íàøëè áàã: {4C75A3}vk.com/w.raines')
    imgui.CenterTextColoredRGB('{ff0000}[+]{ffffff} Àâòîð ñêðèïòà: William_Raines')
    imgui.End()
  end
  ...
  if s_c_window_state.v then
    ...
    imgui.Begin(u8'SWAT FBI | Êîìàíäû ñêðèïòà | Âåðñèÿ: 0.0.2', s_c_window_state, imgui.WindowFlags.NoResize)
    imgui.CenterTextColoredRGB('{ff0000}/cv{ffffff} - Ãîâîðèòü â øèôðîâàííûé êàíàë')
    imgui.CenterTextColoredRGB('{ff0000}/cb{ffffff} - OOC øèôð.êàíàë')
    imgui.CenterTextColoredRGB('{ff0000}/getsquad{ffffff} - Ñïèñîê èãðîêîâ îíëàéí')
    imgui.CenterTextColoredRGB('{ff0000}/sfbi{ffffff} - Âûçâàòü ìåíþ ñêðèïòà')
    imgui.CenterTextColoredRGB('{ff0000}Êëàâèøà «U»{ffffff} - Âûçâàòü ìåíþ ñêðèïòà')
    imgui.End()
  end
  ...
  if ift_window_state.v then
    ...
    imgui.Begin(u8'SWAT FBI | Èíôîðìàöèÿ äëÿ ñòàæ¸ðîâ | Âåðñèÿ: 0.0.2', ift_window_state, imgui.WindowFlags.NoResize)
    imgui.BeginChild('Selectors', imgui.ImVec2(230, 450), true)
      if imgui.Selectable(u8'Îáùåå ïîëîæåíèå', is_genRule) then
        is_genRule = true
        is_wtInfo = false
      ...
      if imgui.Selectable(u8'Èíôîðìàöèÿ î øèôð.êàíàëå', is_wtInfo) then
        is_genRule = false
        is_wtInfo = true
        is_reportRules = false
        is_priorityTasks = false
      ...
      if imgui.Selectable(u8'Ïðàâèëà äîêëàäîâ â øèôð.êàíàë', is_reportRules) then
        is_genRule = false
        is_wtInfo = false
        is_reportRules = true
        is_priorityTasks = false
      ...
      if imgui.Selectable(u8'Ïðèîðèòåòû äëÿ áîéöîâ', is_priorityTasks) then
        is_genRule = false
        is_wtInfo = false
        is_reportRules = false
      ...
      if imgui.Selectable(u8'Áîéöàì S-FBI ðàçðåøåíî', is_sfbiAllowedToDo) then
        is_genRule = false
        is_wtInfo = false
        is_reportRules = false
        is_priorityTasks = false
  ...
  if codes_window_state.v then
    imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
    imgui.SetNextWindowSize(imgui.ImVec2(1150, 500), imgui.Cond.FirstUseEver)
    imgui.Begin(u8'SWAT FBI | Òåí êîäû | Âåðñèÿ: 0.0.2', codes_window_state, imgui.WindowFlags.NoResize)
    imgui.BeginChild('Selectors2', imgui.ImVec2(230, 450), true)
      if imgui.Selectable(u8'Êîäû ðóêîâîäñòâà', is_codeFromStaff) then
        is_codeFromStaff = true
        is_codeFromSquadmate = false
      end
      if imgui.Selectable(u8'Êîäû ïàòðóëåé', is_codeFromSquadmate) then
  ...
  if protocol_window_state.v then
    imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
    imgui.SetNextWindowSize(imgui.ImVec2(1150, 500), imgui.Cond.FirstUseEver)
    imgui.Begin(u8'SWAT FBI | Òåí êîäû | Âåðñèÿ: 0.0.2', protocol_window_state, imgui.WindowFlags.NoResize)
    imgui.BeginChild('Selectors2', imgui.ImVec2(230, 450), true)
      if imgui.Selectable(u8'Ïðîòîêîë îòðÿäà', is_protocol) then
        is_protocol = true
      end
    imgui.EndChild()
  ...
  if actings_window_state.v then
    imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
    imgui.SetNextWindowSize(imgui.ImVec2(300, 300), imgui.Cond.FirstUseEver)
    imgui.Begin(u8'SWAT FBI | Îòûãðîâêè | Âåðñèÿ: 0.0.2', actings_window_state, imgui.WindowFlags.NoResize)
    if imgui.ButtonHex(u8'Çàìàñêèðîâàòü ñåáÿ', 0x6468e8, imgui.ImVec2(285, 0)) then
      lua_thread.create(
        function ()
          sampSendChat('/me ñîðâàë ñ ôîðìû øåâðîíû è çíàêè îòëè÷èé, çàòåì áðîñèë èõ íà çåìëþ')
          wait(7000)
          sampSendChat('/do Íåèçâåñòíûé îäåò â êàìóôëÿæ ò¸ìíî-çåë¸íîãî öâåòà.')
          wait(7000)
          sampSendChat('/do Íà òóëîâèùå âèñèò áðîíåæèëåò V êëàññà çàùèòû.')
          wait(7000)
          sampSendChat('/do Íà ãîëîâå êàñêà «OPS-CORE FAST» è äîï.ïðèáîðû ñâÿçè.')
          wait(7000)
          sampSendChat('/do Îïîçíàâàòåëüíûõ çíàêîâ èëè øåâðîíîâ íà ôîðìå íå èìååòñÿ.')
          wait(7000)
          sampSendChat('/do Ëèöî çàêðûâàåò áàëàêëàâà è î÷êè. Ëè÷íîñòü íå ðàñïîçíàòü.')
        end
      )
    end
    if imgui.ButtonHex(u8'Íàäåòü ïðîòèâîãàç', 0x6468e8, imgui.ImVec2(285, 0)) then
    ...
    if imgui.ButtonHex(u8'Áðîñèòü ÑØÃ ñ âåðòîë¸òà', 0x6468e8, imgui.ImVec2(285, 0)) then
      lua_thread.create(function ()
        sampSendChat('/me äîñòàë èç ïîä ñèäåíüÿ ïðèáîð äëÿ îïðåäåëåíèÿ êîîðäèíàò')
        wait(3600)
        sampSendChat('/do Ïðèáîð îïðåäåëèë êîîðäèíàòû ïîõèòèòåëåé: 2736.1374.1749.9563.')
        wait(3600)
        r_service.sendIC('Êîîðäèíàòû ïîõèòèòåëåé: 2736.1374.1749.9563. Ãîòîâëþñü ðàáîòàòü ÑØÃ.')
        wait(3600)
        sampSendChat('/me äîñòàë èç-çà ïîÿñà ìàëåíüêèå ãðóçèêè, ïîñëå ÷åãî ïðèêðåïèë èõ ê ñâåòîøóìîâîé ãðàíàòå')
        wait(3600)
        sampSendChat('/me ñëåãêà ïðèîòêðûë çàäâèæíóþ äâåðü âåðòîëåòà')
        wait(3600)
        r_service.sendIC('Áðîñàþ ÑØÃ.')
        wait(3600)
        sampSendChat('/me êèäàåò ñâåòîøóìîâóþ ãðàíàòó âíèç')
        wait(3600)
        sampSendChat('/cv Ñáðîñèë ÑØÃ, 8 ñåêóíä')
        wait(3600)
        sampSendChat('/do Â óøè áîéöà âñòàâëåíû Ì-çâóîêîïîäàâëÿþùèå áåðóøè, ãëóøàùèå çâóê ïðè 180+ äÁ.')
        wait(3600)
        sampSendChat('/do Íà áîéöå íàäåòû ñïåö.î÷êè, ïîäàâëÿþùèå ÿðêèå âñïûøêè ñâåòà îò 7 äî 30 ìëí. êÄ.')
        wait(3600)
        sampSendChat('/do ÑØÃ óïàëè ñ âåðòîëåòîâ âîçëå ïåðåãîâîðùèêà è ïîõèòèòåëåé, è Ó-Ä ïðèíöèïîì äåéñòâèÿ âçîðâàëèñü.')
      end)
    end
    imgui.End()
  end
...
function waitForId()
  ...
  chat.sendMessage('Ïðîèñõîäèò ïîäãðóçêà äàííûõ ñîòðóäíèêîâ. Ïîæàëóéñòà ïîäîæäèòå.')
...
function onIRCMessage(user, channel, message)
  if user then
    ...
    chat.sendWarning('Íå óäàëîñü èçâëå÷ü äàííûå.')
  end
end
...
function onIRCDisconnect(message, error)
  if error then
    ...
    if connected then
      connected = false
      user:disconnect()
      chat.sendWarning('Âû áûëè îòêëþ÷åíû îò ÷àòà çà äîëãèé ÀÔÊ. Ïðîáóåì ïåðåïîäêëþ÷èòüñÿ ê ðàöèè.')
      chat.sendWarning('Ýêðàí ïîäâèñíåò íà 5-10 ñåêóíä èç-çà çàãðóçêè äàííûõ. Ïîæàëóéñòà ïîäîæäèòå.')
    end
  end
end
...
function r_service.sendIC(params)
  if connected and not connecting then
    ...
    chat.sendWarning(string.format('Íå óäàëîñü ïîäêëþ÷èòüñÿ ê ðàöèè %s. Ïðîáóåì ïåðåïîäêëþ÷èòüñÿ', SQUAD_NAME))
    r_service.connect()
  end
end
...
function r_service.main()
  while true do
    if connected and not connecting then
      ...
      chat.sendWarning(string.format('Ïîäêëþ÷åíèå ê ðàöèè %s âûïîëíåíî.', SQUAD_NAME))
      r_service.connect()
    end
  end
end
...
function main()
  ...
  and not staff_codes and not protocol then
    regulations = { [1] = 'Îøèáêà çàãðóçêè.' }
    wt_info = { [1] = 'Îøèáêà çàãðóçêè.' }
    wt_report_rules = { [1] = 'Îøèáêà çàãðóçêè.' }
    wt_priority_tasks = { [1] = 'Îøèáêà çàãðóçêè.' }
    sfbi_allowed_toDo = { [1] = 'Îøèáêà çàãðóçêè.' }
    squad_codes = { [1] = 'Îøèáêà çàãðóçêè.' }
    staff_codes = { [1] = 'Îøèáêà çàãðóçêè.' }
    protocol = { [1] = 'Îøèáêà çàãðóçêè' }
    chat.sendWarning('Íå óäàëîñü çàãðóçèòü âàæíóþ èíôîðìàöèþ. Îáðàòèòåñü ê ðàçðàáîò÷èêó.')
  end
  ...
  imgui.Process = false
  chat.sendMessage('Ðàçðàáîò÷èê:{ab0519} William Raines{ffffff}.')
  chat.sendMessage('Îòäåëüíàÿ áëàãîäàðíîñòü {ab0519}Donny Provenzano{ffffff}, {ab0519}Gabriel Marques{ffffff} è {ab0519}Kevin Norberg{ffffff}.')
  chat.sendMessage('{ab0519}Âåðñèÿ ñêðèïòà: {ffffff}0.0.2')
  chat.sendMessage('Ñêðèïò çàãðóæåí.')
  chat.sendMessage('Äëÿ âûçîâà ìåíþ ñêðèïòà èñïîëüçóéòå êëàâèøó {f5001d}«U»{ffffff}.')
